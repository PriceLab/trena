...
2. Create a `CandidateFilter` object based on your desired filtering criteria.
3. Call the `getCandidates` method on your `CandidateFilter` to determine your set of candidate regulators.
4. Create a `Solver` object based on your desired feature selection method using the expression matrix, a target gene of interest, and your candidate regulators. 
5. Call the `run` method on your `Solver` to perform feature selection, identifying putative transcriptional regulators for the target gene.

The filtering steps (Step 2-3), while technically optional, can constrain the solution space and reduce computational time by reducing the number of predictors supplied to the solver. Rather than supplying hundreds of transcription factors that can obscure meaningful relationships between genes, filters allow the user to screen out less likely candidates, leaving only the most likely affectors. 
...
In addition to the databases, constructing a FootprintFilter object requires at least one of 2 different specs for determining the region to look in:

* `geneCenteredSpec` - a list containing: 
    1. `targetGene` - the gene of interest
    2. `tssUpstream` - an integer indicating how many bases upstream of a transcription start site to include in the results
    3. `tssDownstream` - an integer indicating how many bases downstream of a transcription start site to include in the results 
* `regionsSpec` - a list of regions in the DNA where each element is formatted as "chromosome:start-end" 

Note that you can specify both input types, though we anticipate that for most cases, you will only want one of these two types. As with the `VarianceFilter`, the `FootprintFilter` return a named list where one element is a vector containing the names of the canidate regulators. The other element is a table of metadata about footprints, including the motifs, their locations, and the transcription factors to which they link.

For our examples, we'll first run using the `geneCenteredSpec` option:
...
Then, we'll use the `regionsSpec` option to return the same information by supplying the specific transcription start site for MEF2C:
...
The FootprintFilter makes use of another class in the package, the FootprintFinder. This class provides an interface to PostgreSQL and SQLite databases of a specific 2-table schema and has a family of methods for accessing footprint data. To demonstrate some of this functionality, we will use the FootprintFinder to extract footprints from a particular region. In the previous example, we used an SQLite database included in the package; essentially a small snapshot of a larger dataset. More usefully, there are now publically available databases of footprints for use at [bddsrds.globusgenomics.org](). For this exmple, we will access footprints for brain tissue obtained using the HINT method for a seed size of 20 bp; other methods, issues, and alignments are also available. We can access this information as follows (this example will take a bit longer):
...
In general, you shouldn't have much need to use a `FootprintFinder` object directly for TReNA; rather, the `FootprintFilter` object should suffice. 
