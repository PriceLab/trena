...
There are two stages in every trena analysis:
  ...
  2. An ensemble of independent feature selectors (lasso, ridge, xgboost, randomForest
     pearson & spearan correlation) score the association in gene expresssion between
     each transcription factor and the target gene.
...
### [Case Study 1:  trena reproduces known regulation of NFE2 by GATA1 in erythropoiesis](caseStudyOne.html)

The first criterion of value for a computational tool such as trena is its ability to reliably reproduce
existing knowledge.   Here (follow the link) we accomplish that, in these steps:

   1. A summary of the publications which establish this regulatory relationship
   2. Weakly reproduce the finding  using GTEx blood RNA-seq, multiple TF selection strategies
   3. Credibly reproduce the finding use Corces et al, 2016, RNA-seq on FACS-separated erythropoiesis stages, GSE74246
